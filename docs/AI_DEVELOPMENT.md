...
## 6. プロジェクト間の同期と「AI人格」の引き継ぎ

子プロジェクト（実案件）で開発している際も、このワークフローを遵守してください。基盤部分の改善があれば、積極的にマスターリポジトリ（`api-server-test`）へフィードバックすることで、開発チーム全体の「知能」が向上します。

### AIエージェントの「初期化（ブートストラップ）」
新しいプロジェクトをテンプレートから作成して開始する際、AIエージェントの挙動を過去のプロジェクトと一致させる（「物忘れ」を防ぐ）ために、以下の手順を推奨します。

1.  **指示**: 最初のメッセージで「このプロジェクトの `SKILL.md` と `docs/AI_DEVELOPMENT.md` を読み込み、定義されている『AIインスペクター』などの各役割を有効化して。」と伝えてください。
2.  **記憶の同期**: 過去のプロジェクトで得た「進め方のコツ（例：テストの命名規則や、特定ライブラリの癖）」がある場合は、マスターの `docs/AI_DEVELOPMENT.md` の「5. 人間（マネージャー）へのアドバイス」セクションに追記してからテンプレートを更新してください。

これにより、AIはプロジェクトが変わっても「このチームの憲法」を即座に理解し、一貫したインスペクション（監視）とサポートを提供できるようになります。

**Checked for consistency across SKILL, Docs, Code, and Journal (walkthrough.md).**
