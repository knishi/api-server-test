...
## 4. コーディング・運用標準
- **ログ**: `oslo.log` を使用し、相関ID（リクエスト追跡用）を含めます。
- **エラー**: 辞書を返さず、必ず `AppError` 派生の例外を投げます。
- **構成**: Backend (Port 8080) と Frontend (Port 3000) は、ブラウザのセキュリティ制限（CORS/OPTIONS）を考慮した「完全疎結合」を推奨構成とします。
